...
# 2026-07-20 調整 (Tetsuo 指示): 0.30 → 0.40 → 0.50 と段階緩和 (まだ厳しかったため)。
#   BPM60で±0.50s, BPM90で±0.333s(約0.5拍), BPM120で±0.25s, BPM180で±0.167s。
#   timing 判定を寛容化 → rhythmAccuracy / overallScore 上昇。
#   ダウンストリーム: 演奏マスター(直近5回平均90)到達が容易化 / RHYTHM 課題化が減少。
#   過寛容(ズレてても◎になる)が目立てば 0.40 へ戻す。
...
try:
    ...
    # noteStats (音名別・音域帯・遷移・ポジション別) は段5 (2026-09-05 ノート属性ストア) で廃止。
    # 読み手は PerformanceNote → ScoreNote → NoteProfile の明細からその場で作る (app/_libs/noteStoreSummary.ts)。
