...
def position_by_letter(
    ...
    ♭を1つ下の枠に寄せる補正は入れない (2026-09-13 Tetsuo確定「場合による」)。
      E線の A♭5 を指1 は G♯5 と同じ場所なので第2が正しいが、
      変ロ長調・G線の B♭3 を指1 は第2の枠そのもので、下げると誤りになる。
      ♭が枠を動かすかは調と文脈しだいで一律には決まらないため、
      音名の文字だけで数える。
    """
...
def derive_position(
    ...
    positions = [
        p for f in fingers
        if (p := position_by_letter(step, octave, f, string_id)) is not None
    ]
