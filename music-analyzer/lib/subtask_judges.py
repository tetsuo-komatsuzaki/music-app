...
【このファイルの役割 — 他の人が読むときはここだけ理解すればOK】
  **弓（bowing）の点は本ファイル群が唯一の計算元**。
  弦ごとの安定性・弦移動・奏法品質（スタッカートの短さ等）を判定して
  bowing スコア(0-100)を作り、bowingAccuracy として保存する。

  【マスター判定との関係】かつては overallScore=(音程+リズム+弓)÷3 の平均90点だったが、
  overallScore は 2026-06-07 廃止。現行マスターは (音程+リズム)/2 の直近5回平均≥90 で
  弓は含まれない（lib/achievement.py が判定元）。
