...
for m in range(84, 94):
    ...
    check(f"{disp} 指なし", infer_pitch_only(m, step=step, octave=octv),
          (s_exp, p_exp, f_exp, "estimated"))

# ============ D. 指番号ゴールデン (30) ============
results.append("=== D. 指番号あり（音名算術ゴールデン） ===")
D = [  # (midi, step, octave, finger, expected)
    (71,"B",4,1,("A",1,"high")),   # シ4 指1 → A線1ポジ(1指=シ)
    (72,"C",5,2,("A",1,"high")),   # ド5 指2 → A線1ポジ(1指=シ)
    (74,"D",5,3,("A",1,"high")),   # レ5 指3 → A線1ポジ(シドレ=123)
    (76,"E",5,4,("A",1,"high")),   # ミ5 指4 → A線1ポジ
    (77,"F",5,3,("A",3,"high")),   # ★ファ5 指3 → A線3ポジ(1指=レ) Tetsuoの例
    (77,"F",5,1,("E",1,"high")),   # ファ5 指1 → E線1ポジ
    (77,"F",5,2,("A",4,"high")),   # ファ5 指2 → A線4ポジ(1指=ミ)
    (77,"F",5,4,("A",2,"high")),   # ファ5 指4 → A線2ポジ(1指=ド,ドレミファ=1234)
    (79,"G",5,4,("A",3,"high")),   # ソ5 指4 → A線3ポジ(レミファソ=1234)
    (79,"G",5,1,("E",2,"high")),   # ソ5 指1 → E線2ポジ
    (81,"A",5,3,("E",1,"high")),   # ラ5 指3 → E線1ポジ(ファソラ=123)
    (83,"B",5,4,("E",1,"high")),   # シ5 指4 → E線1ポジ
    (84,"C",6,4,("E",2,"high")),   # ド6 指4 → E線2ポジ(1指=ソ)
    (84,"C",6,3,("E",3,"high")),   # ド6 指3 → E線3ポジ(ラシド=123)
    (84,"C",6,2,("E",4,"high")),   # ド6 指2 → E線4ポジ(1指=シ)
    (84,"C",6,1,("E",5,"high")),   # ド6 指1 → E線5ポジ(1指=ド)
    (88,"E",6,4,("E",4,"high")),   # ミ6 指4 → E線4ポジ(1指=シ5)
    (91,"G",6,4,("E",6,"high")),   # ソ6 指4 → E線6ポジ(1指=レ6)
    (93,"A",6,3,("E",8,"high")),   # ラ6 指3 → E線8ポジ(1指=ファ6)
    (96,"C",7,4,("E",9,"high")),  # ド7 指4 → E線9ポジ・弦一意=高信頼
    (100,"E",7,4,("E",11,"high")),# ミ7 指4 → E線11ポジ・一意
    (103,"G",7,4,("E",13,"high")),# ソ7 指4 → E線13ポジ・指板の端。上限14で解ける (2026-09-12)
    ...
    (64,"E",4,1,("D",1,"high")),   # ミ4 指1 → D線1ポジ (G線5ポジも候補)
    (72,"C",5,1,("A",2,"high")),   # ド5 指1 → A線2ポジ(1指=ド)
]
...
check("F5（ファ5）指3・直前A線3ポジ", infer_with_finger(77,3,"A",3,step="F",octave=5), ("A",3,"high"))
...
check("C6（ド6）指なし・直前E線2ポジ", infer_pitch_only(84,"E",2,step="C",octave=6), ("E",2,4,"estimated"))
...
check("D7（レ7）指1 → E線13ポジ (上限14で解ける)", infer_with_finger(98,1,step="D",octave=7), ("E",13,"high"))
check("G#3（ソ♯3）指1 → G線1ポジ(ハーフ扱い)", infer_with_finger(56,1,step="G",octave=3), ("G",1,"high"))
check("D#4（レ♯4）指1 → D線1ポジ(ハーフ扱い)or他弦", infer_with_finger(63,1,step="D",octave=4), ("D",1,"high"))
check("A#4（ラ♯4）指1 → A線1ポジ(ハーフ扱い)or他弦", infer_with_finger(70,1,step="A",octave=4), ("A",1,"high"))
