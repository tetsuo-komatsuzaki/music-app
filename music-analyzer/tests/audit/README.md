...
| `triage_undetected.py` | 「検出できず」の分解ロジック 最終版 (v18 実装 = 文書 v19)。探索ログを捕まえ、第1層 (探索の死角) と第2層 (演奏の事実) に分け、交差窓・境界の印と機械集計を出す。批判者との17往復の結果: https://claude.ai/code/artifact/34be94fe-2590-48cc-9d72-ab86a6489970 |
| `ab_v1_full.py` | 改善案の効果を実測。本体の関数を実行時に差し替えて解析を通しで走らせ、現行と音単位で差分を取る (状態の遷移・リズムの分母・長さの分布・位置合わせ)。解析器のファイルは書き換えない |
| `ab_test_fixes.py` | 同じ方式で 3 変種 (終端のみ / 出だしのみ / 両方) を比べる |
| `check_gate_and_len.py` | ノイズゲートの床が位置合わせを左右するかの確認と、長さが変わる音の変更前の分布 |
| `side_effects.py` | 位置合わせを広げたときの一致率の形・採点への影響・隣り合う音の高さの差 |
