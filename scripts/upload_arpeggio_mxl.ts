// scripts/upload_arpeggio_mxl.ts
// Usage: npx tsx scripts/upload_arpeggio_mxl.ts
// prisma/data/mxl_arpeggio/ 以下の .mxl を Supabase にアップロードし
// prisma/data/mxl_arpeggio_urls.json にパスを保存する
...
const MXL_DIR      = path.join(__dirname, "..", "prisma", "data", "mxl_arpeggio")
const OUTPUT_JSON  = path.join(__dirname, "..", "prisma", "data", "mxl_arpeggio_urls.json")
const CONCURRENCY  = 5
...
async function main() {
  ...
  for (let i = 0; i < files.length; i += CONCURRENCY) {
    ...
    const promises = batch.map(async (filename) => {
      ...
      const storagePath = `practice-items/arpeggios/${filename}`
