...
async function main(){
  ...
  console.log(`移調変種 再生成: ${ids.length}件 (score_full, 2.5s間隔)`)
  let ok=0,err=0
  for(let i=0;i<ids.length;i++){
    ...
    try{
      const res=await fetch(`${RELAY_URL}/invoke`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${API}`},
        body:JSON.stringify({mode:"score_full",idempotency_key:`regen_oct:${id}:${ts}`,practice_item_id:id})})
      if(res.ok) ok++; else { err++; if(err<=10) console.log(`  ERR ${id} [${res.status}] ${(await res.text()).slice(0,70)}`) }
    }catch(e:any){ err++ }
