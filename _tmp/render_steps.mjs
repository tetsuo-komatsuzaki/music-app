...
for (let i = 0; i < N; i++) {
  ...
  await page.waitForTimeout(i === 2 || i === 9 ? 4200 : 900)
  const ph = await page.locator(".phone").first()
  await ph.screenshot({ path: `${G}/shots/steps/step_${String(i).padStart(2, "0")}.png` })
}
await browser.close()
console.log("done", N)
