...
スコープ確定 (2026-08-15 Tetsuo・選択式で個別決定):
1. **区間録音・パート練習も初版で対応** (区間内のみフロー・区間端で自動停止・区間ハイライトは帯座標)
2. **注釈・先生の添削マークも帯レイアウト用に座標再計算して表示**
3. **曲・教材の両方が対象**
4. **iPhone + iPad の両方を検証対象** (審査提出範囲は別途判断)

実装原則 (影響精査 2026-08-15 より):
- 時刻→譜面位置の共通ロジックを1関数に抽出し、縦(Web)/横(アプリ)は投影のみ変える (二重化禁止)
- 現行の縦ネイティブ録音経路は無傷のフォールバックとして残す (回転ロック失敗→縦で続行)
- 回転unlockは停止・エラー・割り込み・アンマウントの全経路で保証
