...
/* ============================================================
   アルコちゃん — 全30ポーズ パラメトリック実装 (C1・2026-07-11)
   出典: Tetsuo提供 ArcoChan.tsx(モーション付きイラスト正本)をTypeScript化。
   ギャラリーは /onboarding/_dev へ分離。SVG+CSSのみ・追加依存ゼロ。
   prefers-reduced-motion で全モーション停止(指示書§2の必須要件)。
   オンボーディングは POSE_KEY_MAP の9キーで参照する(プロトタイプ準拠)。
   ============================================================ */
...
type BowDef = { stick: string; hair: string; frog: [number, number] }
type EyesDef =
  | { type: "open"; cx: [number, number]; cy: number }
  | { type: "happy" }
  | { type: "sad" }
  | { type: "flat" }
type FxDef = {
  ...
  tear?: [number, number]
  lines?: string[]
  confetti?: Array<[number, number, number, string]>
  zzz?: boolean
  bang?: [number, number]
  question?: boolean
}
export interface ArcoPose {
  id: string
  label: string
  cat: string
  anim: string
...
const HandPoint = ({ at }: { at: [number, number] | null }) =>
  at ? (
...
const ArmPath = ({ d }: { d: string }) => <path d={d} fill={C.fur} />
...
const Brows = ({ type }: { type: keyof typeof BROWS }) => {
  const b = BROWS[type]
  return (
    <g id="brows">
      {b.d.map((d, i) => (
...
const Note = ({ x, y, s = 1 }: { x: number; y: number; s?: number }) => (
...
const PROPS = { mug: Mug, bignote: BigNote, sheet: Sheet } as const

function Effects({ fx }: { fx?: FxDef }) {
  if (!fx) return null
  return (
    <g id="fx">
...
const BOW_BACK: BowDef = { stick: "M 152.0,208.4 Q 95.0,179.4 34.8,151.7", hair: "M 154.18,203.90 Q 97.18,174.90 36.98,147.20", frog: [147.0, 201.4] }
const BOW_BACK2: BowDef = { stick: "M 152.0,209.7 Q 95.0,180.7 35.5,153.1", hair: "M 154.18,205.20 Q 97.18,176.20 37.68,148.60", frog: [147.0, 202.7] }
const BOW_BACK3: BowDef = { stick: "M 154.0,209.7 Q 96.0,180.7 33.5,153.1", hair: "M 156.13,205.17 Q 98.13,176.17 35.63,148.57", frog: [149.0, 202.7] }

/* ---------- 全30ポーズ (Tetsuo正本より逐語) ---------- */

export const POSES: ArcoPose[] = [
  /* --- 指差しガイド --- */
...
export const POSE_BY_ID: Record<string, ArcoPose> = Object.fromEntries(
  POSES.map((p) => [p.id, p]),
)

/* オンボーディング9キー → 30ポーズID (プロトタイプの ARCO_POSES と1:1対応) */
export const POSE_KEY_MAP = {
  entry: "09C",    // ジャンプ登場 (SCR-01)
  greet: "09A",    // ぺこりお辞儀 (SCR-02)
  explain: "05B",  // ここポイント！ (SCR-03)
  question: "05C", // 楽譜を見せる (質問画面共通)
  listen: "08B",   // 座ってうっとり (複数選択ゲート)
...
/* ---------- モーション (正本より逐語。reduced-motionで全停止) ---------- */
...
export function ArcoChan({
  ...
  const bowOnWavingSide = !!(p.bow && p.bow.frog[0] < CENTER)
  ...
  const wavingArm = <ArmPath d={p.arms[0]} />
  const stillArm = <ArmPath d={p.arms[1]} />
