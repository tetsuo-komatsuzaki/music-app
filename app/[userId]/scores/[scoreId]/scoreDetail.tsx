...
export default function ScoreDetail({
  ...
  const bestPitchScore = useMemo(() => {
    const scores = performances
      .map(p => {
        if (!p.comparisonResult) return null
        const normalized = normalizeComparison(p.comparisonResult)
        if (!normalized) return null
        const evaluated = normalized.filter(n =>
          n.evaluation_status === "evaluated" || n.evaluation_status === "pitch_only"
        )
        if (evaluated.length === 0) return null
        const ok = evaluated.filter(n => n.pitch_ok === true).length
  ...
  const handleRecordingComplete = useCallback(async (blob: Blob) => {
    ...
    // 2. WAV → uploadAction（scoreId / practiceItemId の両方をセット）
    const wavBlob = await convertRes.blob()
    const wavFile = new File([wavBlob], "recording.wav", { type: "audio/wav" })
    const uploadForm = new FormData()
    uploadForm.set("scoreId", score.id)        // uploadRecord 用
    uploadForm.set("practiceItemId", score.id) // uploadPracticeRecord 用
    ...
    // 3. 最新 2件の結果取得
    try {
      const res = await fetch(`/api/score-performances?scoreId=${score.id}&limit=2`)
      if (!res.ok) throw new Error("fetch failed")
      const perfs = await res.json()
      const latest   = perfs[0]
      const previous = perfs[1]

      const pitchAccuracy  = latest?.pitchAccuracy  ?? null
      const timingAccuracy = latest?.timingAccuracy ?? null
      const prevPitch = previous?.pitchAccuracy ?? bestPitchScore ?? null
      const isPersonalBest = pitchAccuracy != null && (prevPitch == null || pitchAccuracy > prevPitch)
      ...
      return {
        ...
        result: {
          ...
          isPersonalBest,
          previousScore:  prevPitch ?? undefined,
  ...
  }, [score.id, uploadAction, bestPitchScore, router])
  ...
  // --- 比較色の適用（Animation 8: stagger per note） ---
  const applyComparisonColors = useCallback(() => {
    ...
    elements.forEach((el, i) => {
      const r = cmap.get(i)
      if (!r) return
      const color = getComparisonColor(r)
      if (prefersReduced) {
        colorizeNote(el, color)
      } else {
        const delay = Math.min(i * 18, 800)
        setTimeout(() => colorizeNote(el, color), delay)
      }
    })
  }, [comparison])
