...
export default function ScheduleClient({ userId, lessons }: { userId: string; lessons: Lesson[] }) {
  ...
  const inp: React.CSSProperties = { border: "1px solid #dfe3e8", borderRadius: 8, padding: "8px 10px", fontSize: 13 }
  ...
  return (
    <div>
      ...
      <h1 style={{ fontSize: 18, fontWeight: 900, margin: "6px 0 2px" }}>レッスン枠</h1>
      <p style={{ fontSize: 12, color: "#6b7885", margin: "0 0 14px" }}>空き枠を出すと、担当の生徒が予約できます。</p>

      <div style={card}>
        <div style={{ fontSize: 12, fontWeight: 800, color: "#2b3742", marginBottom: 8 }}>空き枠を追加</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
          <input type="datetime-local" value={start} onChange={(e) => setStart(e.target.value)} style={inp} />
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <label style={{ fontSize: 12, color: "#6b7885" }}>長さ
          ...
          <button type="button" onClick={add} disabled={pending}
            style={{ border: "none", borderRadius: 9, padding: 11, fontSize: 13, fontWeight: 800, color: "#fff", background: "#2b3742", cursor: "pointer", opacity: pending ? 0.6 : 1 }}>
            枠を追加
          </button>
        </div>
      </div>

      <div style={{ fontSize: 12, fontWeight: 800, color: "#6b7885", margin: "4px 0 8px" }}>これからのレッスン</div>
      {lessons.length === 0 ? (
        <div style={{ ...card, textAlign: "center", color: "#9aa6b3", fontSize: 12.5 }}>まだ枠がありません。</div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {lessons.map((l) => (
            <div key={l.id} style={card}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 13.5, fontWeight: 800, color: "#2b3742" }}>{l.when}</span>
                <span style={{ fontSize: 11, fontWeight: 800, color: l.status === "booked" ? "#2e8b57" : "#b7823a", flex: "none" }}>
                  {l.status === "booked" ? `予約済み・${l.studentName ?? ""}` : "空き"}
                </span>
              </div>
              <div style={{ fontSize: 11.5, color: "#6b7885", marginTop: 3 }}>
                {l.durationMin}分 ・ {l.online ? "オンライン" : "対面"}{l.locationNote ? ` ・ ${l.locationNote}` : ""}
              </div>
              <button type="button" onClick={() => remove(l.id, l.status === "booked")}
                style={{ marginTop: 8, border: "1px solid #e2e6ea", background: "#fff", color: "#c0473a", borderRadius: 8, padding: "6px 12px", fontSize: 11.5, fontWeight: 800, cursor: "pointer" }}>
                取り消す
              </button>
            </div>
          ))}
