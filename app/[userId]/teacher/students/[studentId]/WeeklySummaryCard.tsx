...
const WORSE = ["rgba(232,138,111,.4)", "#e8a78f"]
const BETTER = ["rgba(168,201,127,.35)", "#a8c97f"]
...
export default function WeeklySummaryCard({ data, karteTabHref }: { data: WeeklySummaryData; karteTabHref: string }) {
  ...
  const svg = useMemo(() => {
    ...
    return (
      <svg viewBox="-24 -27 300 58" role="img" aria-label="音のへんかマップ" style={{ width: "100%", height: "auto", fontFamily: "sans-serif", display: "block" }}>
        ...
        {STRINGS.map((s, si) => (
          <text key={s} x={-H_OPEN - 4} y={-(colX(-H_OPEN, si) + colX(-H_OPEN, si + 1)) / 2 + 2.2} fontSize={6.5} textAnchor="middle" fill="var(--text-ink)">{s}</text>
        ))}
  ...
  const deltaBadge = (d: number): React.CSSProperties => ({
    ...
    color: d < 0 ? "#e8697a" : "#a8c97f", background: d < 0 ? "#fdeceb" : "#e9f8f0",
  })

  return (
    <div style={{ background: "var(--card-in)", border: "1px solid rgba(150,175,225,.12)", borderRadius: 14, overflow: "hidden", marginBottom: 4 }}>
      <div style={{ background: "rgba(232,178,60,.10)", color: "var(--gold)", padding: "9px 13px", display: "flex", alignItems: "baseline", gap: 8 }}>
        <b style={{ fontSize: "var(--fs-caption)", fontWeight: 900 }}>アルコの週間サマリー</b>
        <span style={{ fontSize: "var(--fs-label)", color: "var(--text-muted)" }}>{data.rangeLabel}・先週とくらべて</span>
      </div>
      <div style={{ padding: "9px 13px 12px" }}>
        ...
        <div style={rowS}>
          ...
          <span style={{ fontSize: "var(--fs-caption)", fontWeight: 800, color: "var(--text-ink)", minWidth: 0 }}>
            {data.count === 0 ? "今週はまだ練習なし" : <>
              <b>{data.practicedDays}日・{data.count}回</b>
              {Math.abs(data.count - data.prevCount) >= 2 && (
                <span style={{ fontWeight: 900, color: data.count > data.prevCount ? "#a8c97f" : "#e8697a" }}>
              ...
              <span style={{ display: "inline-flex", gap: 3, marginLeft: 7, verticalAlign: "-2px" }}>
                {data.days.map((on, i) => (
                  <span key={i} style={{ width: 11, height: 11, borderRadius: 3, background: on ? "#7fa4e8" : "rgba(150,175,225,.12)", display: "inline-block" }} />
                ))}
              </span>
            </>}
          </span>
        ...
        {data.perTarget.length > 0 && (
          <div style={{ ...rowS, borderTop: "1px dashed rgba(150,175,225,.12)", alignItems: "flex-start" }}>
            <span style={{ ...rl, paddingTop: 2 }}>練習したもの</span>
            <span style={{ display: "flex", flexWrap: "wrap", gap: 5, minWidth: 0 }}>
              {data.perTarget.map((t, i) => (
                <span key={i} style={{ fontSize: "var(--fs-label)", fontWeight: 800, color: "var(--text-sub)", background: "rgba(150,175,225,.1)", border: "1px solid transparent", borderRadius: 999, padding: "2px 9px" }}>
        ...
        {data.newThings.map((t, i) => (
          <div key={i} style={{ ...rowS, borderTop: "1px dashed rgba(150,175,225,.12)" }}>
            <span style={rl}>あたらしいこと</span>
            <span style={{ fontSize: "var(--fs-caption)", fontWeight: 800, color: "var(--text-ink)" }}>{t}</span>
          </div>
        ))}

        {/* 音のへんか */}
        <div style={{ borderTop: "1px dashed rgba(150,175,225,.12)", paddingTop: 7 }}>
          ...
          {data.changes.length === 0 ? (
            ...
            <>
              <div style={{ background: "#fbfdff", border: "1px solid rgba(150,175,225,.16)", borderRadius: 10, padding: "6px 8px" }}>{svg}</div>
              <div style={{ display: "flex", gap: 12, fontSize: "var(--fs-label)", color: "var(--text-muted)", marginTop: 4 }}>
                <span><i style={{ display: "inline-block", width: 9, height: 9, borderRadius: 2, background: "#e8a78f", marginRight: 3, verticalAlign: "-1px" }} />先週より崩れた</span>
                <span><i style={{ display: "inline-block", width: 9, height: 9, borderRadius: 2, background: "#a8c97f", marginRight: 3, verticalAlign: "-1px" }} />先週より良くなった</span>
              </div>
                ...
                {data.changes.map((c) => (
                  <div key={c.cellId} onClick={() => setSel(c.cellId)}
                    style={{ display: "flex", alignItems: "center", gap: 7, fontSize: "var(--fs-caption)", padding: "4px 4px", borderRadius: 7, cursor: "pointer",
                      background: sel === c.cellId ? "rgba(150,175,225,.12)" : "transparent", outline: sel === c.cellId ? "1px solid rgba(150,175,225,.3)" : "none" }}>
                    <b style={{ flex: "none" }}>{c.label}</b>
                    <span style={{ flex: 1, minWidth: 40, height: 7, borderRadius: 3, background: "rgba(150,175,225,.12)", overflow: "hidden", position: "relative" }}>
                      <span style={{ position: "absolute", top: 0, bottom: 0, borderRadius: 3,
                        left: `${Math.min(c.prevPct, c.nowPct)}%`, width: `${Math.abs(c.delta)}%`,
                        background: c.delta < 0 ? (Math.abs(c.delta) >= 30 ? "#e8a78f" : "rgba(232,138,111,.4)") : (Math.abs(c.delta) >= 30 ? "#a8c97f" : "rgba(168,201,127,.35)") }} />
                    </span>
                    <span style={{ flex: "none", fontWeight: 900, fontVariantNumeric: "tabular-nums", fontSize: "var(--fs-label)" }}>
                      {c.prevPct}%→<span style={{ color: c.delta < 0 ? "#e8697a" : "#a8c97f" }}>{c.nowPct}%</span>
                    </span>
              ...
              {selRow && (
                <div style={{ background: "#f7fafd", border: "1px solid rgba(150,175,225,.16)", borderRadius: 9, padding: "8px 11px", fontSize: "var(--fs-label)", marginTop: 6, lineHeight: 1.7 }}>
                  ...
                  <br /><Link href={karteTabHref} style={{ color: "#7fa4e8", fontWeight: 900, textDecoration: "none" }}>音程マップでくわしく →</Link>
                </div>
              )}
            </>
          )}
