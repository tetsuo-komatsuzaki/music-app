...
// アカウント情報 (2026-08-17 ナビ刷新でプロフィールへ移設)。
// 表示名・メールアドレス・パスワード・退会 をここに集約する。
// 旧・設定画面の「アカウント情報」「アカウント管理」セクションをそのまま移植した
// (処理と文言は変更していない)。設定画面にはアプリ設定だけが残る。
    ...
    <>
      <section className={styles.card}>
        ...
        <div className={styles.field}>
          ...
          {isTooLong && <p className={styles.fieldError}>30 文字以内で入力してください</p>}
        </div>
        ...
        <div className={styles.field}>
          ...
          {!emailEditing ? (
            <button type="button" onClick={() => setEmailEditing(true)} className={styles.secondaryButton}>
              変更
            </button>
          ) : (
            <div className={styles.editForm}>
              ...
              <div className={styles.formActions}>
                ...
                <button type="button" onClick={handleEmailCancel} disabled={isEmailPending} className={styles.secondaryButton}>
                  キャンセル
                </button>
              </div>
            </div>
          ...
          {emailMessage && (
            <p className={emailMessage.type === "success" ? styles.messageSuccess : styles.messageError}>
              {emailMessage.text}
            </p>
          )}
        </div>
        ...
        <div className={styles.field}>
          ...
          {!passwordEditing ? (
            <button type="button" onClick={() => setPasswordEditing(true)} className={styles.secondaryButton}>
              パスワードを変更
            </button>
          ) : (
            <div className={styles.editForm}>
              ...
              <div className={styles.formActions}>
                ...
                <button type="button" onClick={handlePasswordCancel} disabled={isPasswordPending} className={styles.secondaryButton}>
                  キャンセル
                </button>
              </div>
            </div>
        ...
        {message && (
          <p className={message.type === "success" ? styles.messageSuccess : styles.messageError}>{message.text}</p>
        )}
      </section>
