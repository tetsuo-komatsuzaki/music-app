...
export default function PlanCard({ billingEnabled, isPlus, planStatus, periodEnd, trialEligible }: Props) {
  ...
  return (
    <section style={{ background: "#fff", border: "1px solid #eceff3", borderRadius: 14, padding: "16px 18px", marginBottom: 14 }}>
      <h2 style={{ fontSize: 15, fontWeight: 800, margin: "0 0 4px", color: "#22303c" }}>プラン</h2>
      ...
      {isPlus ? (
        <>
          ...
          <button
            ...
            style={{ fontSize: 12.5, fontWeight: 800, color: "#3c4653", background: "#fff", border: "1px solid #dfe3e8", borderRadius: 9, padding: "9px 18px", cursor: "pointer", opacity: pending ? 0.5 : 1 }}
          >
        ...
        <>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button
              ...
              onClick={() => jump("month")}
              disabled={pending != null}
              style={{ fontSize: 12.5, fontWeight: 800, color: "#fff", background: "#8a5a1f", border: "none", borderRadius: 9, padding: "10px 20px", cursor: "pointer", opacity: pending ? 0.5 : 1 }}
            >
            ...
            <button
              ...
              onClick={() => jump("year")}
              disabled={pending != null}
              style={{ fontSize: 12.5, fontWeight: 800, color: "#8a5a1f", background: "#fdf3d8", border: "1px solid #eed9a0", borderRadius: 9, padding: "10px 20px", cursor: "pointer", opacity: pending ? 0.5 : 1 }}
            >
              {pending === "year" ? "開いています…" : "年額 9,800円 (2ヶ月分お得)"}
            </button>
          </div>
          <p style={{ fontSize: 10.5, color: "#9aa6b3", margin: "8px 0 0" }}>
            価格は税込です。いつでも解約できます。
          </p>
        </>
      )}
